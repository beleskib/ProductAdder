...
class MainActivity : AppCompatActivity() {
    ...
    //3
    private fun saveCategories(state: (Boolean) -> Unit) {
        val names = binding.edName.text.toString().trim()
        val rank = binding.edCategory.text.toString().trim()
        val imgCategory = mutableListOf<String>()
        ...
        lifecycleScope.launch {
            ...
            firestore.collection("Categories").add(categories).addOnSuccessListener {
                state(true)
                hideLoading()
